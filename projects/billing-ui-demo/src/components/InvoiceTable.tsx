...
export function InvoiceTable({ invoices, onEdit, onDelete }: Props) {
  ...
  const sorted = useMemo(
    () => sortBy(filtered, sort.key, sort.dir),
    [filtered, sort]
  );

  const selectedInvoice =
    sorted.find((inv) => inv.id === selectedId) ?? null;
  ...
  return (
    <section className="card">
      <div className="row-between table-header">
        <div>
          <h2>Invoices</h2>
          <p className="muted">Filter + sort + modal CRUD (sanitized demo)</p>
        </div>
        ...
        <div className="selected-actions">
          ...
          <button
            className="danger"
            disabled={!selectedInvoice}
            onClick={() => selectedInvoice && onDelete(selectedInvoice)}
          >
            🗑 Delete
          </button>
        </div>
      ...
      <div className="table-wrap">
        <table>
          ...
          <tbody>
            ...
            {sorted.length === 0 && (
              <tr>
                <td colSpan={7}>No results. Try changing filters.</td>
              </tr>
            )}
          </tbody>
        </table>
